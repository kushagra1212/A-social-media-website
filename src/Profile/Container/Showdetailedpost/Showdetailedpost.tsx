...
import { PUBLIC_URL, REACT_APP_CURURL, API_END_POINT } from '../../../utils/constants/env';
  ...
  useEffect(() => {
    const deletePostHandle = async () => {
      ...
      fileRef
        .delete()
        .then(async () => {
          const res = await axios.delete(`${URL}/post/deleteuserpost/${deletePost._id}`);
      ...
      {showShare ? (
        ...
        <div className={Styles.maincontent}>
          ...
          <div key={post._id} className={Styles.singlecontainer}>
            ...
            <a.button style={showShareA} className={Styles.imgdiv}>
              <img src={post.picture ? post.picture : 'userImage.png'} width="100%" alt=" " />
            </a.button>
            ...
            <div style={{ fontSize: '1.3em' }} className={Styles.caption}>
              {post.desc}
            </div>
          </div>
        </div>
